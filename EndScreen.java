...
public class EndScreen extends JPanel implements ActionListener {
	// issues:
	/*
	 * i really just need to test the stuff and of course the rotating text
	 */
	...
	public void paintComponent(Graphics g) {
		...
		for (int count = 0; count < 5; count++) {
			int first = count + 1;
			int second = count + 6;
			g.drawString(first + ": " + minList.get(first - 1), (int) (width * .13),
					(int) (height * .55 + height * .06 * count)); // figure out where to store values
			g.drawString(second + ": " + minList.get(second - 1), (int) (width * .71),
					(int) (height * .55 + height * .06 * count));
		}
		...
		g.setColor(new Color(96, 96, 96));
		g.fillRoundRect((int) (width * .385) - 5, (int) (height * .72) + 7, (int) (width * .23), (int) (height * .17),
				(int) (width * .015), (int) (width * .015));
		g.fillRoundRect((int) (width * .385) - 5, (int) (height * .49) + 7, (int) (width * .23), (int) (height * .17),
				(int) (width * .015), (int) (width * .015));
		g.setColor(new Color(178, 0, 0));
		g.fillRoundRect((int) (width * .385), (int) (height * .49), (int) (width * .23), (int) (height * .17),
				(int) (width * .015), (int) (width * .015));
		g.fillRoundRect((int) (width * .385), (int) (height * .72), (int) (width * .23), (int) (height * .17),
				(int) (width * .015), (int) (width * .015));
	}
	...
	public EndScreen(double score, ArrayList<Double> topTen, Game game) { // not sure what topTen should be
		...
		replay.setBackground(new Color(178, 0, 0));
		...
		this.add(replay);
		Score = new JButton("Score: " + score);
		Score.setBackground(new Color(178, 0, 0));
