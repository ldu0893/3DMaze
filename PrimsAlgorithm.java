...
//			frontier.add(new Position(size-2, size-1, size-1));
	...
	private void addFrontier(boolean[][][] visited, ArrayList<Position> frontier, Position newRoom) {
		Position[] possibleFrontier = { new Position(newRoom.x + 1, newRoom.y, newRoom.z),
				new Position(newRoom.x - 1, newRoom.y, newRoom.z), new Position(newRoom.x, newRoom.y + 1, newRoom.z),
				new Position(newRoom.x, newRoom.y - 1, newRoom.z), new Position(newRoom.x, newRoom.y, newRoom.z + 1),
				new Position(newRoom.x, newRoom.y, newRoom.z - 1) };
		for (Position pos : possibleFrontier) {
			// System.out.println("frontier.contains(newRoom): " +
			// frontier.contains(newRoom));
	...
	private void connectRoom(Room[][][] maze, boolean[][][] visited, Position newRoom) {
		...
		for (Integer direction : order) {
			...
			case 0: // connect E
				...
				if (connectingRoom0.isValid(size) && visited[connectingRoom0.x][connectingRoom0.y][connectingRoom0.z]) {
			...
			case 1: // connects W
				...
				if (connectingRoom1.isValid(size) && visited[connectingRoom1.x][connectingRoom1.y][connectingRoom1.z]) {
			...
			case 2: // connects N
				...
				if (connectingRoom2.isValid(size) && visited[connectingRoom2.x][connectingRoom2.y][connectingRoom2.z]) {
			...
			case 3: // connects S
				...
				if (connectingRoom3.isValid(size) && visited[connectingRoom3.x][connectingRoom3.y][connectingRoom3.z]) {
			...
			case 4: // connects U
				...
				if (connectingRoom4.isValid(size) && visited[connectingRoom4.x][connectingRoom4.y][connectingRoom4.z]) {
			...
			case 5: // connects D
				...
				if (connectingRoom5.isValid(size) && visited[connectingRoom5.x][connectingRoom5.y][connectingRoom5.z]) {
